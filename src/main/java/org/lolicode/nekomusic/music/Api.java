...
//          String qrimg;
    ...
    public static boolean genLoginKey() {
        try (Response response = NekoMusic.HTTP_CLIENT.newCall(new Request.Builder()
            ...
            if (response.code() == 200 && response.body() != null) {
                key = NekoMusic.GSON.fromJson(response.body().string(), KeyObj.class).data.unikey;
                return true;
            } else {
                NekoMusic.LOGGER.error("Failed to generate login key, response code: " + response.code());
            }
        } catch (IOException e) {
            NekoMusic.LOGGER.error("Failed to generate login key", e);
        }
        return false;
    }
    ...
    public static String genLoginQrcode() {
        ...
        try (Response response = NekoMusic.HTTP_CLIENT.newCall(new Request.Builder()
                ...
                .build()).execute()) {
            if (response.code() == 200 && response.body() != null) {
                String qrurl = NekoMusic.GSON.fromJson(response.body().string(), QrCodeObj.class).data.qrurl;
            ...
            NekoMusic.LOGGER.error("Failed to generate login qrcode: " + response.code());
        } catch (IOException | WriterException e) {
            NekoMusic.LOGGER.error("Failed to generate login qrcode", e);
        }
        return null;
    }
    ...
    public static int checkLoginStatus() {
        ...
        try (Response response = NekoMusic.HTTP_CLIENT.newCall(new Request.Builder()
                ...
                .build()).execute()) {
            if (response.code() == 200 && response.body() != null) {
                CookieObj cookieObj = NekoMusic.GSON.fromJson(response.body().string(), CookieObj.class);
                if (cookieObj.code == LOGIN_STATUS.SUCCESS) {
                    NekoMusic.CONFIG.cookie = cookieObj.cookie;
                    ModConfig.save();
                    return LOGIN_STATUS.SUCCESS;
                } else {
                    NekoMusic.LOGGER.error("Failed to check login status: " + cookieObj.code);
                    return cookieObj.code;
                }
            } else {
                NekoMusic.LOGGER.error("Failed to check login status: " + response.code());
            }
        } catch (IOException e) {
    ...
    public static UserInfo.Data.Profile getUserInfo() {
        ...
        try (Response response = NekoMusic.HTTP_CLIENT.newCall(new Request.Builder()
                ...
                .build()).execute()) {
            if (response.code() == 200 && response.body() != null) {
                UserInfo userInfo = NekoMusic.GSON.fromJson(response.body().string(), UserInfo.class);
                if (userInfo.data.code == 200) {
                    return userInfo.data.profile;
                } else {
                    NekoMusic.LOGGER.error("Failed to get user info: " + userInfo.data.code);
                }
            } else {
                NekoMusic.LOGGER.error("Failed to get user info: Invaild response: " + response.code());
            }
        } catch (IOException e) {
    ...
    private static MusicObj getMusic(long id) {
        if (id == 0) return null;
        HttpUrl.Builder url = HttpUrl.parse(NekoMusic.CONFIG.apiAddress + "/song/url").newBuilder()
                .addQueryParameter("id", String.valueOf(id))
                .addQueryParameter("br", String.valueOf(NekoMusic.CONFIG.maxQuality));
        ...
        try (Response response = NekoMusic.HTTP_CLIENT.newCall(new Request.Builder()
                ...
                .execute()) {
            if (response.code() == 200 && response.body() != null) {
                MusicObjWrapper music = NekoMusic.GSON.fromJson(response.body().string(), MusicObjWrapper.class);
                if (music.data != null && music.data.size() > 0) {
                    return music.data.get(0);
                }
            } else {
                NekoMusic.LOGGER.error("Failed to get music: Invalid response " + response.code());
            }
        } catch (IOException e) {
    ...
    public static MusicObj getMusicInfo(long id) {
        ...
        try (Response response = NekoMusic.HTTP_CLIENT.newCall(new Request.Builder()
                ...
                .execute()) {
            if (response.code() == 200 && response.body() != null) {
                MusicInfoWrapper music = NekoMusic.GSON.fromJson(response.body().string(), MusicInfoWrapper.class);
                if (music.songs != null && music.songs.length > 0) {
                    return music.songs[0];
                }
            } else {
                NekoMusic.LOGGER.error("Failed to get music info: Invalid response " + response.code());
            }
        } catch (IOException e) {
            NekoMusic.LOGGER.error("Failed to get music info: Network error", e);
        }
        return null;
    }
    ...
    public static SongList getSongList(long id) {
        if (id == 0) return null;
        HttpUrl.Builder url = HttpUrl.parse(NekoMusic.CONFIG.apiAddress + "/playlist/track/all").newBuilder()
                .addQueryParameter("id", String.valueOf(id));
        if (NekoMusic.CONFIG.cookie != null && !NekoMusic.CONFIG.cookie.isEmpty()) {
            url.addQueryParameter("cookie", NekoMusic.CONFIG.cookie);
        }
        try (Response response = NekoMusic.HTTP_CLIENT.newCall(new Request.Builder()
                ...
                .execute()) {
            if (response.code() == 200 && response.body() != null) {
                SongList list = NekoMusic.GSON.fromJson(response.body().string(), SongList.class);
                list.id = id;
                return list;
            } else {
                NekoMusic.LOGGER.error("Failed to get songs: Invalid response " + response.code());
            }
        } catch (IOException e) {
            NekoMusic.LOGGER.error("Failed to get songs: Network error", e);
        }
        return null;
    }
    ...
    public static SearchResult search(String keyword, int page, int limit) {
        ...
        HttpUrl.Builder url = HttpUrl.parse(NekoMusic.CONFIG.apiAddress + "/search").newBuilder()
        ...
        try (Response response = NekoMusic.HTTP_CLIENT.newCall(new Request.Builder()
                ...
                .execute()) {
            if (response.code() == 200 && response.body() != null) {
                SearchResult result = NekoMusic.GSON.fromJson(response.body().string(), SearchResult.class);
                result.result.page = page;
                result.result.keyword = keyword;
                return result;
            } else {
                NekoMusic.LOGGER.error("Failed to search songs: Invalid response " + response.code());
            }
        } catch (IOException e) {
